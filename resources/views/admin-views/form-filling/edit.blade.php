                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            <li class="breadcrumb-item"><a class="breadcrumb-link"
                                                           href="">{{\App\CPU\translate('Form Filling')}}</a>
                            </li>
                ...
                <div class="card mt-3">
                    ...
                    <form id="form-filling">
                        ...
                        <label for="patient_last_visit_id" class="col-form-label input-label">{{\App\CPU\translate('Form Type')}}<i class="tio-help-outlined text-body ml-0" data-toggle="tooltip" data-placement="top" title="Display doc_zipcode"></i></label>
                            ...
                            <select name="diagnose[]" class="form-control form-filling-field" id="dignoseSelect" multiple>
                                <option value="Back">Back</option>
                                <option value="Neck">Neck</option>
                                <option value="Both Knee">Both Knee</option>
                                <option value="Right Knee">Right Knee</option>
                                <option value="Left Knee">Left Knee</option>
                                <option value="Both Shoulder">Both Shoulder</option>
                                <option value="Right Shoulder">Right Shoulder</option>
                                <option value="Left Shoulder">Left Shoulder</option>
                                <option value="Both Ankle">Both Ankle</option>
                                ...
                                <option value="Both Elbow">Both Elbow</option>
                                <option value="Left Elbow">Left Elbow</option>
                                <option value="Right Elbow">Right Elbow</option>
                                
                                <option class="commercial {{ $lead->form_filling_details ? ($lead->form_filling_details->type == 'Universal' ? '' : 'd-none') : 'd-none' }}" value="Commercial Both Knee">Commercial Both Knee</option>
                                <option class="commercial {{ $lead->form_filling_details ? ($lead->form_filling_details->type == 'Universal' ? '' : 'd-none') : 'd-none' }}" value="Commercial Left Knee">Commercial Left Knee</option>
                                <option class="commercial {{ $lead->form_filling_details ? ($lead->form_filling_details->type == 'Universal' ? '' : 'd-none') : 'd-none' }}" value="Commercial Right Knee">Commercial Right Knee</option>
                            </select>
                            @php
...
@push('script_2')
   <script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('submit','#form-filling',function(e){
            e.preventDefault();
            var selectElement = document.getElementById("dignoseSelect");
            var selectedOptions = selectElement.selectedOptions;
            if (selectedOptions.length === 0) {
                swal.fire("Warning!","Please select at least one diagnose.","warning");
                return false;
            }else{
            $('.submit-btn').attr('disabled',true);
            $('.form-control').removeClass('is-invalid');
            $('.invalid-feedback.is-invalid').remove();
            $.ajax({
                url : "{{route('admin.form_filling.store')}}",
                type : "POST",
                data : new FormData(this),
                dataType : "json",
                contentType : false,
